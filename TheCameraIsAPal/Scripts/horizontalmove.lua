...
--                      Animation/SkidTurn against the real rig and ship as
--                      AnimSequences, so the play path wraps them in a
--                      dynamic montage. Their beats are cut to the phase
--                      boundaries below: plant at trigger, hero pose mid
--                      SKID, catch on the SKID -> LAUNCH handoff, exit as
--                      the turn releases.
...
-- The pivot curve is also an ANIMATION decision, not just a feel one: it
-- decides how much of the turn the body has already made while the skid pose
-- is on screen. EaseOutCirc over 0.24s is 69% round by the plant frame and
-- 95% by the hero frame, i.e. the character faces the new heading while still
-- sliding along the old one, and a leg thrown out front reads as a lunge
-- rather than a brake. EaseInOutSine over 0.30s is 12% / 59% at those frames,
-- which is the mid-rotation window the authored clip is staged for.
...
-- -> pak pipeline). Either asset type works: an AnimMontage is played
-- directly, an AnimSequence is wrapped in a dynamic montage on SKID_SLOT,
-- which is what the build pipeline produces and needs no montage asset
-- authored in-editor.
local SKID_MONTAGES = {
...
local SKID_PLAY_RATE = 1.00   -- clips are authored at their real speed
...
-- play call handed back, because the dynamic-montage path builds its montage
