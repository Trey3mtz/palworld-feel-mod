...
# ---------------------------------------------------------------------------
# generic writer: an arbitrary node tree, in an arbitrary FBX axis system
# ---------------------------------------------------------------------------

def write_generic(path, clip_name, names, parents, frames, fps=30.0, settings=None,
                  kinds=None):
    """Write animation for a node tree that is not our rig.

    frames[f][i] = (translation3, euler3_degrees, scale3), already in the
    TARGET file's own space -- no conversion is applied here. That is the
    point: when the clip is retargeted onto a skeleton from someone else's
    FBX, writing it in that file's conventions means Unreal's importer does
    the same conversion to both, so the two cannot disagree.

    settings: GlobalSettings overrides (UpAxis, FrontAxis, UnitScaleFactor...)
    taken from the skeleton file, so the axis header matches too.
    """
    ids = _Ids()
    n_nodes = len(names)
    n_frames = len(frames)
    ...
    axis.update(settings or {})

    lines = []
    w = lines.append
    now = time.gmtime()
    w("; FBX 7.4.0 project file")
    w("; Generated by Animation/SkidTurn -- retargeted skeleton-only animation")
    w("; ----------------------------------------------------")
    w("")
    w("FBXHeaderExtension:  {")
    w("\tFBXHeaderVersion: 1003")
    ...
    w("\tCreationTimeStamp:  {")
    w("\t\tVersion: 1000")
    for key, val in (("Year", now.tm_year), ("Month", now.tm_mon), ("Day", now.tm_mday),
    ...
    w("\tVersion: 1000")
    w("\tProperties70:  {")
    for key in ("UpAxis", "UpAxisSign", "FrontAxis", "FrontAxisSign", "CoordAxis",
                "CoordAxisSign", "OriginalUpAxis", "OriginalUpAxisSign"):
        val = axis.get(key, axis.get(key.replace("Original", ""), 0))
        w('\t\tP: "%s", "int", "Integer", "",%d' % (key, int(val)))
    w('\t\tP: "UnitScaleFactor", "double", "Number", "",%s' % _fmt(axis["UnitScaleFactor"]))
    w('\t\tP: "OriginalUnitScaleFactor", "double", "Number", "",%s' % _fmt(axis["UnitScaleFactor"]))
    w('\t\tP: "TimeSpanStart", "KTime", "Time", "",0')
    w('\t\tP: "TimeSpanStop", "KTime", "Time", "",%d' % stop)
    w('\t\tP: "TimeMode", "enum", "", "",%d' % TIME_MODE_30FPS)
    w('\t\tP: "CustomFrameRate", "double", "Number", "",%s' % _fmt(fps))
    ...
    w("Definitions:  {")
    w("\tVersion: 100")
    w("\tCount: %d" % (1 + n_nodes * 2 + 2 + n_curvenodes + n_curves))
    for otype, count in (("GlobalSettings", 1), ("NodeAttribute", n_nodes),
                         ("Model", n_nodes), ("AnimationStack", 1),
                         ("AnimationLayer", 1), ("AnimationCurveNode", n_curvenodes),
                         ("AnimationCurve", n_curves)):
    ...
    for i, name in enumerate(names):
        ...
        attr_id.append(aid)
        model_id.append(mid)
        w('\tNodeAttribute: %d, "NodeAttribute::%s", "%s" {' % (aid, name, kinds[i]))
        w("\t\tProperties70:  {")
        w('\t\t\tP: "Size", "double", "Number", "",1')
        w("\t\t}")
        w('\t\tTypeFlags: "%s"' % ("Null" if kinds[i] == "Null" else "Skeleton"))
        w("\t}")
        t0, r0, s0 = frames[0][i]
        w('\tModel: %d, "Model::%s", "%s" {' % (mid, name, kinds[i]))
        w("\t\tVersion: 232")
        w("\t\tProperties70:  {")
        w('\t\t\tP: "RotationActive", "bool", "", "",1')
        w('\t\t\tP: "RotationOrder", "enum", "", "",0')
        ...
        w('\t\t\tP: "DefaultAttributeIndex", "int", "Integer", "",0')
        w('\t\t\tP: "Lcl Translation", "Lcl Translation", "", "A",%s,%s,%s'
          % tuple(_fmt(v) for v in t0))
        w('\t\t\tP: "Lcl Rotation", "Lcl Rotation", "", "A",%s,%s,%s'
          % tuple(_fmt(v) for v in r0))
        w('\t\t\tP: "Lcl Scaling", "Lcl Scaling", "", "A",%s,%s,%s'
          % tuple(_fmt(v) for v in s0))
        w("\t\t}")
        w("\t\tShading: T")
        w('\t\tCulling: "CullingOff"')
        w("\t}")
    ...
    curve_nodes, curves = [], []
    for i in range(n_nodes):
        for slot, (prop, label) in enumerate((("Lcl Translation", "T"),
                                              ("Lcl Rotation", "R"),
                                              ("Lcl Scaling", "S"))):
            values = [frames[f][i][slot] for f in range(n_frames)]
            cn_id = ids()
            curve_nodes.append((cn_id, model_id[i], prop))
            w('\tAnimationCurveNode: %d, "AnimCurveNode::%s", "" {' % (cn_id, label))
            w("\t\tProperties70:  {")
            for a, axis_name in enumerate("XYZ"):
                w('\t\t\tP: "d|%s", "Number", "", "A",%s' % (axis_name, _fmt(values[0][a])))
            w("\t\t}")
            w("\t}")
            for a, axis_name in enumerate("XYZ"):
                c_id = ids()
                curves.append((c_id, cn_id, axis_name))
                w('\tAnimationCurve: %d, "AnimCurve::", "" {' % c_id)
                w("\t\tDefault: %s" % _fmt(values[0][a]))
                w("\t\tKeyVer: 4009")
                w("\t\tKeyTime: *%d {" % n_frames)
                w("\t\t\ta: %s" % key_times)
                w("\t\t}")
                w("\t\tKeyValueFloat: *%d {" % n_frames)
                w("\t\t\ta: %s" % ",".join(_fmt(v[a]) for v in values))
                w("\t\t}")
                w("\t\tKeyAttrFlags: *1 {")
                w("\t\t\ta: 4")
                w("\t\t}")
    ...
    w("Connections:  {")
    for i, name in enumerate(names):
        pid = model_id[parents[i]] if parents[i] >= 0 else 0
        w('\t;Model::%s' % name)
        w('\tC: "OO",%d,%d' % (model_id[i], pid))
        w('\tC: "OO",%d,%d' % (attr_id[i], model_id[i]))
    w('\tC: "OO",%d,%d' % (layer_id, stack_id))
    for cn_id, mid, prop in curve_nodes:
        w('\tC: "OO",%d,%d' % (cn_id, layer_id))
        w('\tC: "OP",%d,%d, "%s"' % (cn_id, mid, prop))
    for c_id, cn_id, axis_name in curves:
        w('\tC: "OP",%d,%d, "d|%s"' % (c_id, cn_id, axis_name))
    w("}")
    w("")
